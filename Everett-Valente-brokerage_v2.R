...
#==============================================================================================================================
# Function to calculate Everett and Valente (2016) brokerage scores 
# This version (v2) works for both undirected and directed graphs
...
#   all.values (optional): by default is FALSE; if TRUE it will include all the other calculated values in addition to ev_brokerage values
...
#    res.df <- calculate.EV.brokerage(g)
#    res.g  <- calculate.EV.brokerage(g, as.graph=TRUE)
#    res.df  <- calculate.EV.brokerage(g, all.values=TRUE) #includes other calculated values in addition to ev_brokerage values 
#================================================================================================================================
calculate.EV.brokerage <- function(g, as.graph= FALSE, all.values=FALSE){
  ...
  if (is_directed(g)) {
    ...
    if (vertex_attr_names(res)[1] == "name") 
      res <- select(res, name, ev_brokerage, ev_in, ev_out, in_degree, out_degree, in_reachable, out_reachable, betweenness) 
    else 
      res <- select(res, ev_brokerage, ev_in, ev_out, in_degree, out_degree, in_reachable, out_reachable, betweenness) 
    
    if (all.values==FALSE) 
        res <- select(res, ev_brokerage) 
  ...
  else { #calculate for undirected graph
    ...
    res <- g %>% 
      ...
      mutate(ev_step2 = if_else(is_pendant,                               
             ...
             ev_brokerage = if_else(ev_step2 == 0,                   
                                    ev_step2,                        
                                    ev_step2 / centrality_degree())    #3) Divide each non-zero score by vertex degree
      ) 
    if (vertex_attr_names(res)[1] == "name") {
      res <- select(res, name, ev_brokerage, betweenness, is_pendant) 
      
      if (all.values==TRUE) 
        res <- select(res, name, ev_brokerage, betweenness) 
      else 
        res <- select(res, name, ev_brokerage) 
    }
    else {
      res <- select(res, ev_brokerage, betweenness, is_pendant) 
      
      if (all.values==TRUE) 
        res <- select(res, ev_brokerage, betweenness) 
      else
        res <- select(res, ev_brokerage) 
    }
  }
